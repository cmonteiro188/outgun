...
void MapGenerator::generate(int w, int h, bool allow_over_edge) {
...
void MapGenerator::draw(ostream& out) const {
...
pair<int, int> MapGenerator::max_distance() {
...
int MapGenerator::distance(int sx, int sy, int gx, int gy) {
    if (gx == sx && gy == sy)
        return 0;
...
const pair<int, int>& MapGenerator::find_best(const vector<vector<Node> >& node, const vector<pair<int, int> >& open) {
    int score = INT_MAX;
...
void MapGenerator::save_map(ostream& out, const string& title, const string& author) const {
