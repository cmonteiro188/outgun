...
bool isLocalIP(NLaddress address) throw ();  // returns true if address points to this machine (nothing to do with the address being private)
std::string addressToString(const NLaddress& address) throw ();
inline bool operator==(const NLaddress& a1, const NLaddress& a2) throw () { return nlAddrCompare(&a1, &a2); }
...
NetworkResult save_http_response(NLsocket& socket, std::ostream& out, const volatile bool* abortFlag, int timeout) throw ();   // timeout in ms
