...
public:
    ...
    virtual int disconnect_client(int client_id, int timeout, NLubyte reason, bool fromUserThread = true) throw () = 0;
...
// disabled in Outgun to prevent problems    virtual int broadcast_message(const char* data, int length) = 0;
    ...
    virtual NLaddress get_client_address(int client_id) const throw () = 0;
};
